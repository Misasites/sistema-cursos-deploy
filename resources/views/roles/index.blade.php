                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 col-sm-12 mt-4 pt-3">
                            <button type="submit" class="btn btn-info btn-sm"><i class="fa-solid fa-magnifying-glass"></i>
                                Pesquisa</button>
                            <a href="{{ route('role.index') }}" class="btn btn-danger btn-sm"><i
                                ...
                                <td class="d-md-flex flex-row justify-content-center">
                                    ...
                                    @can('index-role-permission')
                                        <a href="{{ route('role-permission.index', ['role' => $role->id]) }}"
                                            class="btn btn-info btn-sm me-1 mb-1 mb-md-0" title="Permissões"><i class="fa-solid fa-list"></i>
                                            </a>
                                    @endcan

                                    @can('edit-role')
                                        <a href="{{ route('role.edit', ['role' => $role->id]) }}"
                                            class="btn btn-warning btn-sm me-1 mb-1 mb-md-0" title="Editar">
                                    ...
                                    @can('destroy-role')
                                        <form method="POST" action="{{ route('role.destroy', ['role' => $role->id]) }}">
                                            @csrf
                                            @method('delete')
                                            <button type="submit" class="btn btn-danger btn-sm me-1 mb-1 mb-md-0" title="Apagar"
